...
def insert_list_activity(request):
    if request.method == 'GET':
        ...
        try:
            ...
            return Response({"result": f"Inserido com sucesso com id {new_activity_id}"}, status=status.HTTP_201_CREATED)
        except Exception as e:
...
def insert_list_course(request):
    if request.method == 'GET':
        try:
            ...
            return Response({"result": courses_result['result'], "message": courses_result['message']}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
